...
For proxy-segmented environments where the JFrog source and the ACR target have no shared network path, use the following two-script workflow. Both scripts include integrity validation using `skopeo inspect` layer size comparison, validated in production air-gapped deployments.
...
### Script 2 — Upload and Post-Verify (Proxy OFF, ACR Target)

Run on the target-side jump server inside the secure network. Extracts archives, pushes with `--preserve-digests`, and post-verifies that ACR layer sizes match the local baseline.
...
ACR_NAME="<acr-name>"
TARGET_REGISTRY="${ACR_NAME}.azurecr.io"
REPO="tibco-platform"
...
skopeo login "${TARGET_REGISTRY}" \
  --username "00000000-0000-0000-0000-000000000000" --password "${ACR_TOKEN}"
