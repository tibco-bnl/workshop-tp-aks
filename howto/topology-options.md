...
## Platform-Specific Notes — Azure Kubernetes Service (AKS)

All four topologies are supported on AKS.

### Ingress

AKS uses Traefik or Nginx as the IngressClass (standard Kubernetes Ingress, not OpenShift Routes):

- A wildcard Azure DNS A record pointing to the ingress controller LoadBalancer IP covers all CP and DP hostnames
- Traefik is the recommended IngressClass for TIBCO Platform on AKS
- For Topology 3 (dedicated CP cluster + separate DP clusters), each AKS cluster has its own ingress controller IP and DNS wildcard record

### Container Registry

Use Azure Container Registry (ACR) for private images:

```bash
# Attach ACR to AKS cluster for pull authentication
az aks update --name ${TP_CLUSTER_NAME} \
  --resource-group ${TP_RESOURCE_GROUP} \
  --attach-acr ${TP_CONTAINER_REGISTRY_NAME}
```

Alternatively, use JFrog Artifactory with a `imagePullSecret`.

### Control Tower Connectivity

When Control Tower runs on an AKS cluster, it reaches on-premises BW5/BW6/EMS via:
...
| Use | Storage Class |
|-----|--------------|
| Block (RWO) | `managed-premium` (Azure Disk CSI) |
| File (RWX) | `azurefile-premium` (Azure Files CSI) |

### Database

- **Azure Database for PostgreSQL Flexible Server** (PaaS) — recommended for production
